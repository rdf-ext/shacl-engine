...
class ShapeValidator {
  constructor (shape, context) {
    ...
    this._compiled = once(() => context.validationRegistry.compile(shape, context))
  }
  ...
  async validateNode (context) {
    for (const validation of this.compiled) {
      if (validation.node) {
        await validation.node(context)
      }
    }
  }

  async validateNodes (context) {
    for (const validation of this.compiled) {
      if (validation.nodes) {
        await validation.nodes(context)
      }
    }
  }
  ...
  async validateProperty (context) {
    ...
    if (this.shape.isValueShape) {
      resolved = await this.shape.valuesFunc(context)
    } else if (this.shape.isSparqlShape) {
    ...
    for (const validation of this.compiled) {
      if (validation.values) {
        await validation.values(valuesContext)
      }
    }
    ...
    for (const { value, valuePaths } of valuesPaths) {
      ...
      for (const validation of this.compiled) {
        if (validation.value) {
          await validation.value(valueContext)
        }
      }
    }
  }
