...
const defaultConformanceDisallows = new TermSet([ns.sh.Info, ns.sh.Warning, ns.sh.Violation])
...
class Report {
  constructor ({ details, factory, options, results = [] } = {}) {
    ...
    this._conforms = once(() => !this.results.some(result => {
      if (this.options.conformanceDisallows) {
        return this.options.conformanceDisallows.has(result.severity)
      } else {
        return defaultConformanceDisallows.has(result.severity)
      }
    }))
  ...
  build () {
    ...
    ptr
      ...
      .addOut([ns.sh.conformanceDisallows], this.options.conformanceDisallows || [])
