...
function compileConformsToShape (funcContext) {
  return funcContext.compile([{
    ...nodeExpArg,
    property: ns.shnex.conformsToShape,
    asArray: true
  }], async ({ engine, factory, focusNode }, arg) => {
    const [node, shape] = arg

    if (node.length === 0 || shape.length === 0) {
      return focusNode.node([])
    }

    const report = await engine.validate(node, shape)

    return focusNode.node(factory.literal(report.conforms.toString(), ns.xsd.boolean))
  })
}
