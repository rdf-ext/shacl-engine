...
function compileNode (shape) {
  const node = shape.validator.shape(shape.ptr.out([ns.sh.node]))
...
function validateNode (node) {
  return async context => {
    const nodeContext = await node.validate(context.create({ child: true, focusNode: context.valueOrNode }))

    context.test(nodeContext.report.conforms, ns.sh.NodeConstraintComponent, {
      args: { node: node.ptr.term },
...
function validateProperty (property) {
  return async context => {
    for (const focusNode of context.valueOrNode) {
      const propertyContext = context.create({ focusNode })
