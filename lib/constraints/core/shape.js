...
function compileReifierShape (shape, context) {
  const reifierShape = [...shape.ptr.out([ns.sh.reifierShape])].map(ptr => context.getShape(ptr))
  const reificationRequired = shape.ptr.out([ns.sh.reificationRequired]).terms.some(term => isTrue(term))

  return {
    value: validateReifierShape(reifierShape, reificationRequired)
  }
}

function validateReifierShape (reifierShape, reificationRequired) {
  return async context => {
    for (const valuePath of context.valuePaths) {
      const reifiers = valuePath.reifier()

      for (const reifier of reifiers) {
        for (const shape of reifierShape) {
          const nodeContext = await shape.validateNode(context.create({ child: true, focusNode: reifier, value: null }))

          context.test(nodeContext.report.conforms, ns.sh.ReifierShapeConstraintComponent, {
            args: { node: shape.ptr.term },
            message: [context.factory.literal('Value does not have shape {$node}')],
            results: nodeContext.report.results,
            value: context.valueOrNode
      ...
      if (reificationRequired && reifiers.length === 0) {
        context.violation(ns.sh.ReifierShapeConstraintComponent, {
          message: [context.factory.literal('Required reifier missing')],
          value: context.valueOrNode
        })
      }
    }
