...
function validateAnd (and) {
  return async context => {
    ...
    const result = await every(and, async shape => {
      const childContext = await shape.validateNode(context.create({ child: true, focusNode: context.valueOrNode }))
...
function validateNot (not) {
  return async context => {
    const childContext = await not.validateNode(context.create({ child: true, focusNode: context.valueOrNode }))
